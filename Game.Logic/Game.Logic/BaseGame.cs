...
namespace Game.Logic
{
	public class BaseGame : AbstractGame
	{
		...
		public void ClearAllChild()
		{
			...
			foreach (Living current in this.m_livings)
			{
				if (current.IsLiving && current is SimpleNpc)
				{
					list.Add(current);
				}
			}
				...
				if (arrayList != null && this.GameState != eGameState.Stopped)
				{
					...
					if (arrayList.Count > 0)
					{
						...
						foreach (IAction action in arrayList)
						{
							...
							{
								BaseGame.log.Error("Map update error:", exception);
							}
						}
		...
		internal void SendMissionTryAgain(int needmoney)
		{
			GSPacketIn gSPacketIn = new GSPacketIn(91);
			gSPacketIn.WriteByte(119);
			gSPacketIn.WriteInt(needmoney);
			this.SendToAll(gSPacketIn);
		}
		...
		internal void SendAddLiving(Living living)
		{
			...
			byte val = 0;
			if (living is SimpleBoss || living is SimpleNpc)
			{
				val = 1;
			}
		...
		internal void SendGamePlayerProperty(Living player, string type, string state)
		{
			GSPacketIn gSPacketIn = new GSPacketIn(91, player.Id);
			gSPacketIn.Parameter1 = player.Id;
		...
		internal void SendGameBigBox(Living player, List<int> listTemplate)
		{
			...
			foreach (int current in listTemplate)
			{
		...
		internal void SendPlayerUseProp(Living player, int type, int place, int templateID)
		{
			GSPacketIn gSPacketIn = new GSPacketIn(91, player.Id);
			gSPacketIn.Parameter1 = player.Id;
			gSPacketIn.WriteByte(32);
			...
			gSPacketIn.WriteInt(templateID);
			gSPacketIn.WriteInt(player.Id);
			gSPacketIn.WriteBoolean(templateID == BuffType.Fatal);
			this.SendToAll(gSPacketIn);
		}
		...
		internal void SendGameNextTurn(Living living, BaseGame game, List<Box> newBoxes)
		{
			...
			if (living is TurnedLiving)
			{
				...
				foreach (Player current2 in allFightPlayers)
				{
					...
					gSPacketIn.WriteInt(current2.flyCount);
				}
