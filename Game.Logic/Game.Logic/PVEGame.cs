...
namespace Game.Logic
{
	public class PVEGame : BaseGame
	{
		...
		{
			return this.CreateBoss(npcId, x, y, direction, type, action, false);
		}
		public SimpleBoss CreateBoss(int npcId, int x, int y, int direction, int type, string action, bool isworld)
		{
			NpcInfo npcInfoById = NPCInfoMgr.GetNpcInfoById(npcId);
			SimpleBoss simpleBoss = new SimpleBoss(this.PhysicalId++, this, npcInfoById, direction, type, action);
			simpleBoss.IsWorldBoss = isworld;
			simpleBoss.Reset();
		...
		public void CreateGate(bool isEnter)
		{
			...
			Player player = base.FindRandomPlayer();
			int num = 150;
			int num2 = player.X;
			int y = player.Y;
			if (num2 + num > foregroundWidth)
			{
				num2 -= num;
			}
			else
			...
			Point left = this.m_map.FindYLineNotEmptyPoint(num2, y);
			if (left == Point.Empty)
			{
				left = new Point(num2, base.Map.Bound.Height + 1);
			}
			this.CreatePhysicalObj(left.X, left.Y - 75, "moive", "asset.game.transmitted", "out", 1, 1);
			if (isEnter)
			{
		...
		public override Player RemovePlayer(IGamePlayer gp, bool isKick)
		{
			...
			if (player != null)
			{
				...
				if (player.IsLiving && base.GameState == eGameState.Playing)
				{
					...
					player.PlayerDetail.OutLabyrinth(true);
				}
				else
				{
		...
		public void PrepareNewSession()
		{
			if (base.GameState == eGameState.Prepared || base.GameState == eGameState.GameOver || base.GameState == eGameState.ALLSessionStopped)
			{
				...
				this.Param3 = -1;
				this.Param4 = -1;
				this.Pic = "show" + this.SessionId + ".jpg";
		...
		public void StartGame()
		{
			if (base.GameState == eGameState.Loading)
			{
				...
				foreach (Player current in allFightPlayers)
				{
					...
					gSPacketIn.WriteInt(current.deputyWeaponCount);
					...
					gSPacketIn.WriteInt(0);
					gSPacketIn.WriteInt(0);
					gSPacketIn.WriteBoolean(current.IsFrost);
					gSPacketIn.WriteBoolean(current.IsHide);
					gSPacketIn.WriteBoolean(current.IsNoHole);
					gSPacketIn.WriteInt(0);
				}
				gSPacketIn.WriteInt(0);
				gSPacketIn.WriteInt(0);
			...
			if (DropInventory.CopyDrop(this.m_missionInfo.Id, 1, ref list) && list != null)
			{
				foreach (ItemInfo current in list)
				{
					...
					if (current != null)
					{
						...
						count = current.Count;
						player.PlayerDetail.AddTemplate(current, eBageType.TempBag, current.Count, eItemNotice.GoodsTipBroadcastTypeView, eItemNotice.GoodsTipBroadcastTypeView);
					}
				}
		...
		public List<int> CreateBigBox()
		{
			List<ItemInfo> list = null;
			DropInventory.CopyDrop(this.m_missionInfo.Id, this.SessionId, ref list);
			List<int> list2 = new List<int>();
			if (list != null)
			{
				foreach (ItemInfo current in list)
				{
					list2.Add(current.TemplateID);
				}
			}
			return list2;
		}
		public void calExpLaby(ref int exp, ref int count, ref int labyrinthGold)
		{
			...
			int num2 = (this.SessionId - 1 > array.Length) ? (array.Length - 1) : (this.SessionId - 1);
			num2 = ((num2 < 0) ? 0 : num2);
			exp = array[num2];
			count = array2[num2];
			labyrinthGold = array3[num2];
		}
		public void TakeBigBox()
		{
			List<int> list = this.CreateBigBox();
			List<ItemInfo> list2 = new List<ItemInfo>();
			List<ItemTemplateInfo> list3 = new List<ItemTemplateInfo>();
			foreach (int current in list)
			{
				list3.Add(ItemMgr.FindItemTemplate(current));
			}
			int num = 0;
			int num2 = 0;
			int num3 = 0;
			this.calExpLaby(ref num2, ref num, ref num3);
			foreach (ItemTemplateInfo current2 in list3)
			{
				list2.Add(ItemInfo.CreateFromTemplate(current2, 1, 101));
			}
			string text = "";
			foreach (Player current3 in base.GetAllFightPlayers())
			{
				if (current3.PlayerDetail.isDoubleAward())
				{
					num *= 2;
					num2 *= 2;
					num3 *= 2;
				}
				base.SendGameBigBox(current3, list);
				foreach (ItemInfo current4 in list2)
				{
					current4.IsBinds = true;
					current3.PlayerDetail.AddTemplate(current4, current4.Template.BagType, num, eItemNotice.GoodsTipBroadcastTypeView, eItemNotice.GoodsTipBroadcastTypeView);
					text += string.Format("{0} x{1}, ", current4.Template.Name, num);
				}
				current3.PlayerDetail.AddHardCurrency(num3);
				if (text != "")
				{
					text = text + "Moedas de ouro da terra secreta x" + num3;
				}
				current3.PlayerDetail.AddGP(num2);
				current3.PlayerDetail.SendHideMessage(string.Format("Você ganhou: {0} exp, {1}", num2, text));
			}
		}
		public void GameOverMovie()
		{
			if (base.GameState == eGameState.Playing)
			{
				...
				base.ClearWaitTimer();
				base.ClearDiedPhysicals();
				foreach (Player current in base.GetAllFightPlayers())
				{
					current.PlayerDetail.UpdateLabyrinth(this.SessionId, true);
					if (!this.CanGetBigBox)
					{
						int num = 0;
						int num2 = 0;
						int num3 = 0;
						this.calExpLaby(ref num2, ref num, ref num3);
						if (current.PlayerDetail.isDoubleAward())
						{
							num2 *= 2;
						}
						current.PlayerDetail.AddGP(num2);
						current.PlayerDetail.SendHideMessage(string.Format("Você ganhou: {0} exp", num2));
					}
				}
				List<Physics> allPhysicalSafe = this.m_map.GetAllPhysicalSafe();
				foreach (Physics current2 in allPhysicalSafe)
				{
					current2.PrepareNewTurn();
				}
				this.m_currentLiving = base.FindNextTurnedLiving();
				try
				{
		...
		{
			return this.MissionInfo.Id >= 40001 && this.MissionInfo.Id <= 40030;
		}
		public void GameOver()
		{
			if (base.GameState == eGameState.Playing)
			{
				...
				foreach (Player current in allFightPlayers)
				{
					if (!this.IsWin)
					{
						current.PlayerDetail.OutLabyrinth(this.IsWin);
					}
		...
		public bool TakeBossCard(Player player, int index)
		{
			...
			DropInventory.BossDrop(missionId, ref list);
			if (list != null)
			{
				foreach (ItemInfo current in list)
				{
		...
		public void SendMissionInfo()
		{
			...
			gSPacketIn.WriteInt(0);
			gSPacketIn.WriteString(this.Pic);
			this.SendToAll(gSPacketIn);
		}
