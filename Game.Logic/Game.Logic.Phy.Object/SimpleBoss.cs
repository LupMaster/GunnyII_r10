...
namespace Game.Logic.Phy.Object
{
	public class SimpleBoss : TurnedLiving
	{
		...
		public SimpleBoss(int id, BaseGame game, NpcInfo npcInfo, int direction, int type, string actions) : base(id, game, npcInfo.Camp, npcInfo.Name, npcInfo.ModelID, npcInfo.Blood, npcInfo.Immunity, direction)
		{
			if (type == 0)
			{
		...
		public void CreateBoss(int id, int x, int y, int direction, int disToSecond, int maxCount, string action)
		{
			if (this.CurrentLivingBossNum < maxCount)
			{
				if (maxCount - this.CurrentLivingNpcNum >= 2)
				{
					this.Boss.Add(((PVEGame)base.Game).CreateBoss(id, x + disToSecond, y, direction, 1, action));
					this.Boss.Add(((PVEGame)base.Game).CreateBoss(id, x, y, direction, 1, action));
					return;
				}
