	...
	public class Player : TurnedLiving
	{
		...
		public void InitBuffer(List<ItemInfo> equpedEffect)
		{
			...
			foreach (ItemInfo current in equpedEffect)
			{
				...
				if (num3 == 39)
				{
					this.ReduceCritFisrtGem = num4;
					this.ReduceCritSecondGem = num4;
					int type = runeTemplateInfo.Type2;
					if (this.DefenFisrtGem == 0)
					{
						this.DefenFisrtGem = type;
					}
					else
					{
						this.DefenSecondGem = type;
					}
				}
				if (num3 == 37 || num3 == 39)
				{
		...
		public void OpenBox(int boxId)
		{
			...
			if (box != null && box.Item != null)
			{
				...
				{
					this.m_player.AddTemplate(item, eBageType.FightBag, item.Count, eItemNotice.GoodsTipBroadcastTypeView, eItemNotice.GoodsTipTypeView);
				}
				else
				{
					this.m_player.AddTemplate(item, eBageType.TempBag, item.Count, eItemNotice.GoodsTipBroadcastTypeView, eItemNotice.GoodsTipTypeView);
				}
				IL_167:
		...
		public override void StartAttacking()
		{
			if (!base.IsAttacking)
			{
				if (this.m_Healstone != null && this.m_blood < this.m_maxBlood)
				{
					this.AddBlood(this.m_Healstone.Template.Property2);
					this.m_player.RemoveHealstone();
				}
		...
		public void PetUseKill(int skillID)
		{
			...
			{
				this.m_player.SendMessage("Energia Insuficiente.");
			}
		}
						...
						if (this.CanGetProp)
						{
							...
							if (DropInventory.FireDrop(this.m_game.RoomType, ref list) && list != null)
							{
								foreach (ItemInfo current in list)
								{
									...
									if (current != null && base.vaneOpen)
									{
										this.PlayerDetail.AddTemplate(current, eBageType.FightBag, current.Count, eItemNotice.GoodsTipTypeView, eItemNotice.OpenTypeView);
									}
								}
		...
		public void UseSecondWeapon()
		{
			if (this.CanUseItem(this.m_DeputyWeapon.Template))
			{
				if (this.m_DeputyWeapon.Template.Property3 == 31)
				{
					...
					new AddGuardEquipEffect(count, 2).Start(this);
				}
				else
				{
