...
namespace Game.Server.Packets.Client
{
	...
	public class AuctionAddHandler : IPacketHandler
	{
		public int HandlePacket(GameClient client, GSPacketIn packet)
		{
			...
			if (itemAt == null)
			{
				client.Player.SendMessage("Vật phẩm lạ không tìm thấy!!!");
				return 0;
			}
			int num8 = itemAt.Count - num5;
			if (itemAt.Count < num5 || num5 < 0)
			{
				client.Out.SendMessage(eMessageType.Normal, LanguageMgr.GetTranslation("Số lượng không có thực, thao tác thất bại!", new object[0]));
				return 0;
			}
			if (client.Player.PlayerCharacter.Grade < 20)
			{
				client.Out.SendMessage(eMessageType.Normal, string.Format("Cấp 20 trở lên mới có thể đấu giá vật phẩm.", new object[0]));
				return 0;
			}
			if (num2 < 0)
			{
							...
							else
							{
								...
								using (PlayerBussiness playerBussiness = new PlayerBussiness())
								{
									if (playerBussiness.AddAuction(auctionInfo))
									{
										...
										if (num8 > 0)
										{
											client.Player.AddTemplate(itemAt, bagType, num8, eItemNotice.NoneTypeView, eItemNotice.NoneTypeView);
										}
										client.Player.SaveIntoDatabase();
