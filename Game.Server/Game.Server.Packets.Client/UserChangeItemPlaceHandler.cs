...
namespace Game.Server.Packets.Client
{
	...
	public class UserChangeItemPlaceHandler : IPacketHandler
	{
		public int HandlePacket(GameClient client, GSPacketIn packet)
		{
			...
			if (num3 < 0 || num3 > itemAt.Count || num3 > 999)
			{
				num3 = itemAt.Count;
			}
			...
			if (bag != null && item != null && bag != null)
			{
				...
				if (itemAt != null)
				{
					...
					else
					{
						...
						{
                            if (item.Template.CategoryID != 1 && item.Template.CategoryID != 2 && item.Template.CategoryID != 3 && item.Template.CategoryID != 4 && item.Template.CategoryID != 5 && item.Template.CategoryID != 6 && item.Template.CategoryID != 7 && item.Template.CategoryID != 13 && item.Template.CategoryID != 15)
                            {
                                bag.TakeOutItem(item);
