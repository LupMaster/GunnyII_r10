...
namespace Game.Server.Packets.Client
{
    ...
    public class FigSpiritUpGradeHandler : IPacketHandler
    {
        ...
        {
            return (exp >= FigSpiritUpGradeHandler.exps[1] && _curLv == 0) || (exp >= FigSpiritUpGradeHandler.exps[2] && _curLv == 1) || (exp >= FigSpiritUpGradeHandler.exps[3] && _curLv == 2) || (exp >= FigSpiritUpGradeHandler.exps[4] && _curLv == 3) || (exp >= FigSpiritUpGradeHandler.exps[5] && _curLv == 4) || (exp >= FigSpiritUpGradeHandler.exps[6] && _curLv == 5) || (exp >= FigSpiritUpGradeHandler.exps[7] && _curLv == 6) || (exp >= FigSpiritUpGradeHandler.exps[8] && _curLv == 7) || (exp >= FigSpiritUpGradeHandler.exps[9] && _curLv == 8) || (exp >= FigSpiritUpGradeHandler.exps[10] && _curLv == 9);
        }
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            if (itemCount <= 0)
            {
                ...
                client.Out.SendMessage(eMessageType.Normal, LanguageMgr.GetTranslation("Espiritos Insuficiente!", new object[0]));
                return 0;
            }
            if (DateTime.Compare(client.Player.LastFigUpTime.AddSeconds(2.0), DateTime.Now) > 0)
            {
                client.Player.Out.SendPlayerFigSpiritUp(iD, gemStone, flag, max, isFall, 0, dir);
                client.Out.SendMessage(eMessageType.Normal, LanguageMgr.GetTranslation("Muito Rápido!", new object[0]));
                return 1;
            }
