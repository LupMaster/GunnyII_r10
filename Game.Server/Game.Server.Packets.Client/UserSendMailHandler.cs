...
namespace Game.Server.Packets.Client
{
	...
	public class UserSendMailHandler : IPacketHandler
	{
		public int HandlePacket(GameClient client, GSPacketIn packet)
		{
			...
			int num5 = 0;
			if (num3 != -1 && client.Player.PlayerCharacter.Grade < 20)
			{
				client.Out.SendMessage(eMessageType.Normal, "Cấp 20 trở lên mới có thể gởi file đính kèm.");
			...
			using (PlayerBussiness playerBussiness = new PlayerBussiness())
			{
				...
				if (playerInfo.Grade < 20)
				{
					client.Out.SendMessage(eMessageType.Normal, "Không thể gửi cho nhân vật nhỏ hơn cấp 20.");
					gSPacketIn.WriteBoolean(false);
					client.Out.SendTCP(gSPacketIn);
					int result = 0;
					return result;
					...
					if (playerInfo.NickName != client.Player.PlayerCharacter.NickName)
					{
						...
						if (playerBussiness.SendMail(mailInfo))
						{
							...
							if (itemInfo != null)
							{
								...
								if (num5 > 0)
								{
									client.Player.AddTemplate(itemInfo, bagType, num5, eItemNotice.NoneTypeView, eItemNotice.NoneTypeView);
								}
							}
						}
