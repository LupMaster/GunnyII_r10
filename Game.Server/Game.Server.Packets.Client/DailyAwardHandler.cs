	...
	public class DailyAwardHandler : IPacketHandler
	{
		public int HandlePacket(GameClient client, GSPacketIn packet)
		{
			...
			case 2:
				{
					...
					if (!ItemBoxMgr.CreateItemBox(itemTemplateInfo.TemplateID, list, ref num2, ref num, ref num3, ref num4, ref num5))
					{
			...
			using (ProduceBussiness produceBussiness = new ProduceBussiness())
			{
				...
				if (string.IsNullOrEmpty(text2) || string.IsNullOrEmpty(text2.Split(new char[]
				{
					','
				})[0]))
				{
			...
			foreach (ItemInfo current in list)
			{
				...
				if (!client.Player.AddTemplate(current, current.Template.BagType, current.Count, eItemNotice.NoneTypeView, eItemNotice.NoneTypeView))
				{
