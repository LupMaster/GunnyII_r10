...
namespace Game.Base.Packets
{
	...
	public class AbstractPacketLib : IPacketLib
	{
		...
		public void SendOpenWorldBoss()
		{
            ...
            gSPacketIn.WriteInt(1243);
			gSPacketIn.WriteString("Mundo fechão");
			gSPacketIn.WriteString(array[2]);
			gSPacketIn.WriteLong(RoomMgr.WorldBossRoom.MaxBlood);
			...
			gSPacketIn.WriteBoolean(false);//true
            gSPacketIn.WriteBoolean(false);//true
			...
			gSPacketIn.WriteInt(200);
			gSPacketIn.WriteInt(1);
			gSPacketIn.WriteInt(1);
			gSPacketIn.WriteString("Aumentar o Dano");
			gSPacketIn.WriteInt(30);
            gSPacketIn.WriteString("Aumentar o Dano em 2000000.");
			gSPacketIn.WriteInt(-1);
			gSPacketIn.WriteBoolean(true);
        ...
        public void SendFightFootballTimeActive()
        {
            GSPacketIn gSPacketIn = new GSPacketIn(151);
            gSPacketIn.WriteBoolean(true);
            this.SendTCP(gSPacketIn);
        }
        public GSPacketIn SendNewChikenBoxGame(int ID)
        {
            GSPacketIn gSPacketIn = new GSPacketIn(87, ID);
            gSPacketIn.WriteInt(0);//this._model.canOpenCounts = _loc_2.readInt();// 1 là mở
            for (int i = 0; i < 1; i++)
            {
                gSPacketIn.WriteInt(2);//this._model.openCardPrice.push(_loc_2.readInt());
            }
            gSPacketIn.WriteInt(1);//this._model.canEagleEyeCounts = _loc_2.readInt();
            for (int i2 = 0; i2 < 1; i2++)
            {
                gSPacketIn.WriteInt(2);//this._model.eagleEyePrice.push(_loc_2.readInt());
            }
        ...
        public GSPacketIn SendWonderfulHander(int ID)
        {
            GSPacketIn gSPacketIn = new GSPacketIn(159, ID);
            gSPacketIn.WriteByte(2);//gSPacketIn.WriteInt(0);
            gSPacketIn.WriteInt(1);
            gSPacketIn.WriteInt(0);
		...
		public GSPacketIn SendRoomChange(BaseRoom room)
		{
			...
			gSPacketIn.WriteString((room.Name == null) ? "DDTank" : room.Name);
			gSPacketIn.WriteByte(room.TimeMode);
			gSPacketIn.WriteByte((byte)room.HardLevel);
			gSPacketIn.WriteInt(room.LevelLimits);
			gSPacketIn.WriteBoolean(room.isCrosszone);
		...
		public GSPacketIn SendPetInfo(PlayerInfo info, UsersPetinfo[] pets)
		{
			...
			for (int i = 0; i < pets.Length; i++)
			{
				...
				Dictionary<int, PetEquipDataInfo> equip = usersPetinfo.GetEquip();
				gSPacketIn.WriteInt(equip.Count);
				for (int j = 0; j < equip.Count; j++)
				{
					gSPacketIn.WriteInt(equip[j].eqType);
		...
		public GSPacketIn SendUpdateAllData(PlayerInfo player)//KingBless
		{
            ...
            /*
            int type = 1;
            gSPacketIn.WriteInt(type);//this._openType = _loc_2.readInt();
            gSPacketIn.WriteDateTime(DateTime.Now.AddDays(7.0));//this._endTime = _loc_2.readDate();
            if (type > 0)
            {
                gSPacketIn.WriteInt(0);//_loc_3 = _loc_2.readInt();
                for (int i = 0; i < 1; i++)
                {
                    gSPacketIn.WriteInt(0);//_loc_5 = _loc_2.readInt();
                    gSPacketIn.WriteInt(0);//this._buffData[_loc_5] = _loc_2.readInt();
                }
            }*/
			this.SendTCP(gSPacketIn);
			return gSPacketIn;
        ...
        public GSPacketIn SendUpdateBuffData(PlayerInfo player)
        {
            GSPacketIn gSPacketIn = new GSPacketIn(143, player.ID);
            gSPacketIn.WriteInt(0);//var _loc_3:* = _loc_2.readInt();
            gSPacketIn.WriteInt(0);//this._buffData[_loc_3] = _loc_2.readInt();
            this.SendTCP(gSPacketIn);
            return gSPacketIn;
        }
		...
		public GSPacketIn SendUserRanks(int Id, List<UserRankInfo> ranks)
		{
			...
			foreach (UserRankInfo current in ranks)
			{
				gSPacketIn.WriteString(current.UserRank);
			}
			this.SendTCP(gSPacketIn);
			return gSPacketIn;
		}
		public GSPacketIn SendUpdateAchievements(GamePlayer player, BaseAchievement[] infos)
		{
			...
			{
				BaseAchievement baseAchievement = infos[i];
