...
namespace Game.Server.Battle
{
	public class FightServerConnector : BaseConnector
	{
		...
		public FightServerConnector(BattleServer server, string ip, int port, string key) : base(ip, port, true, new byte[8192], new byte[8192])
		{
		...
		public void SendAddRoom(BaseRoom room)
		{
			...
			foreach (GamePlayer current in players)
			{
				...
				gSPacketIn.WriteString(current.PlayerCharacter.Honor);
				...
				if (current.Healstone != null)
				{
					gSPacketIn.WriteInt(current.Healstone.TemplateID);
					gSPacketIn.WriteInt(current.Healstone.Count);
				}
				else
				{
					gSPacketIn.WriteInt(0);
