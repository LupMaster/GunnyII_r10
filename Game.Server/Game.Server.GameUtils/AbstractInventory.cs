...
namespace Game.Server.GameUtils
{
	public abstract class AbstractInventory
	{
		...
		public virtual void RemoveGoldEquip()
		{
			for (int i = 0; i < this.m_capalility; i++)
			{
				if (this.m_items[i] != null)
				{
					...
					if (itemInfo.IsGold && !itemInfo.IsValidGoldItem())
					{
						itemInfo.IsGold = false;
						itemInfo.StrengthenLevel--;
						GoldEquipTemplateLoadInfo goldEquipTemplateLoadInfo = GoldEquipMgr.FindGoldEquipOldTemplate(itemInfo.TemplateID);
						if (goldEquipTemplateLoadInfo != null && itemInfo.Template.CategoryID == 7)
						{
							ItemTemplateInfo itemTemplateInfo = ItemMgr.FindItemTemplate(goldEquipTemplateLoadInfo.OldTemplateId);
							if (itemTemplateInfo != null)
							{
								ItemInfo itemInfo2 = ItemInfo.CloneFromTemplate(itemTemplateInfo, itemInfo);
								this.RemoveItem(this.m_items[i]);
								this.AddItemTo(itemInfo2, itemInfo2.Place);
								itemInfo = itemInfo2;
							}
						}
