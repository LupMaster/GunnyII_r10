...
namespace Game.Server.GameUtils
{
	public abstract class CardAbstractInventory
	{
		...
		public virtual bool UpdateCard(UsersCardInfo item)
		{
			if (item == null)
			{
				return false;
			...
			try
			{
				...
				{
					if (this.m_cards[i] != null && this.m_cards[i].TemplateID == item.TemplateID)
					{
						num = i;
						this.m_cards[i] = item;
					}
		...
		public UsersCardInfo[] GetRawSpaces()
		{
			...
			try
			{
				result = (this.m_cards.Clone() as UsersCardInfo[]);
			}
			finally
			{
				Monitor.Exit(@lock);
