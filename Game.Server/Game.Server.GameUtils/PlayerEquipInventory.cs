	...
	public class PlayerEquipInventory : PlayerInventory
	{
		...
		public override void UpdateChangedPlaces()
		{
			...
			for (int i = 0; i < array2.Length; i++)
			{
				...
				if (this.IsEquipSlot(slot))
				{
					...
					if (itemAt != null)
					{
						this.m_player.OnUsingItem(itemAt.TemplateID);
						itemAt.IsBinds = true;
						if (!itemAt.IsUsed)
						{
							itemAt.IsUsed = true;
		...
		public void UpdatePlayerProperties()
		{
			...
			try
			{
				...
				List<UsersCardInfo> cards = this.m_player.CardBag.GetCards(0, 5);
				object @lock;
				...
				try
				{
					...
					{
						num16 += this.m_player.Pet.TotalAttack;
						num17 += this.m_player.Pet.TotalDefence;
						num18 += this.m_player.Pet.TotalAgility;
						num19 += this.m_player.Pet.TotalLuck;
						num20 += this.m_player.Pet.TotalBlood;
					}
					UserRankInfo rank = this.m_player.Rank.GetRank(this.m_player.PlayerCharacter.Honor);
					if (rank != null)
					{
						num += rank.Attack;
						num2 += rank.Defence;
				...
				this.m_player.PlayerProp.CreateProp(true, "Texp", num21, num22, num23, num24, num25);
				this.m_player.PlayerProp.CreateProp(true, "Card", num12, num13, num14, num15, 0);
				this.m_player.PlayerProp.CreateProp(true, "Pet", num16, num17, num18, num19, num20);
				this.m_player.PlayerProp.CreateProp(true, "Gem", num7, num8, num9, num10, num11);
