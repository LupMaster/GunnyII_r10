...
namespace Game.Base
{
        ...
        public static int[] SEND_KEY = new int[8]
    {
      ...
      241
        ...
        {
            return Marshal.ConvertToUInt32(this.ReadByte(), this.ReadByte(), this.ReadByte(), this.ReadByte());
        }

        public virtual long ReadLong()
        {
            return Marshal.ConvertToInt64(this.ReadInt(), this.ReadUInt());
        }
        ...
        public virtual int CopyTo3(byte[] dst, int dstOffset, int offset, byte[] key, ref int packetArrangeSend)
        {
            ...
            lock (this)
            {
                if (num > 0)
                {
                    int local_2 = this.m_sended + dstOffset;
                    int local_2_1;
                    if (this.isSended)
                    {
                        ...
                        this.m_sended = 0;
                        this.isSended = false;
                        local_2_1 = this.m_sended + dstOffset;
                    }
                    else
                        local_2_1 = 8192;
                    ...
                    {
                        for (int local_3 = 0; local_3 < num; ++local_3)
                        {
                            int local_4 = offset + local_3;
                            while (local_2_1 > 8192)
                                local_2_1 -= 8192;
                            if (this.m_sended == 0)
                            {
                                dst[dstOffset] = (byte)((uint)this.m_buffer[local_4] ^ (uint)key[this.m_sended % 8]);
                            }
                            else
                            {
                                key[this.m_sended % 8] = (byte)((int)key[this.m_sended % 8] + (int)dst[local_2_1 - 1] ^ this.m_sended);
                                dst[dstOffset + local_3] = (byte)(((uint)this.m_buffer[local_4] ^ (uint)key[this.m_sended % 8]) + (uint)dst[local_2_1 - 1]);
                            }
                            ++this.m_sended;
                            ++local_2_1;
                        }
